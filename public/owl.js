(function (exports) {
    ...
    class Link extends Component {
        constructor() {
            ...
            this.href = this.env.router.info.destToUrl(this.props);
        }
        async willUpdateProps(nextProps) {
    ...
    function activate(env, routes, options) {
        // process routes and build proper internal data structures
        const mode = options ? options.mode : "hash";
        const info = { routes: {}, routeIds: [], mode, destToUrl: destToURL.bind(null, env) };
        let nextId = 1;
        for (let route of routes) {
            ...
            route.params = findParams(route.path);
            info.routes[route.name] = route;
            info.routeIds.push(route.name);
        }
        const router = { navigate, routeName: null, routeParams: {}, info };
        env.router = router;
        checkRoute(env);
        function navigate(dest) {
            const to = destToURL(env, dest);
            history.pushState({}, to, location.origin + to);
            checkAndUpdateRoute(env);
        }
        addEventListener("popstate", () => checkAndUpdateRoute(env));
        // setup link and directive
        env.qweb.addTemplate(LINK_TEMPLATE_NAME, LINK_TEMPLATE);
        QWeb.addDirective(makeDirective(env));
    }
    function checkRoute(env) {
        const info = env.router.info;
        let currentPath = info.mode === "history" ? window.location.pathname : window.location.hash.slice(1);
    ...
    function checkAndUpdateRoute(env) {
    ...
    exports.__info__.date = '2019-07-20T09:17:39.482Z';
    exports.__info__.hash = '45d6b87';
